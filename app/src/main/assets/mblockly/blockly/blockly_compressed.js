	...
	for(var c = 0, d = goog.string.trim(String(a)).split("."), e = goog.string.trim(String(b)).split("."), f = Math.max(d.length, e.length), g = 0; 0 == c && g < f; g++) {
		...
		do {
			...
			var c = 0 == l[1].length ? 0 : parseInt(l[1], 10),
				p = 0 == q[1].length ? 0 : parseInt(q[1], 10),
				c = goog.string.compareElements_(c, p) || goog.string.compareElements_(0 ==
					l[2].length, 0 == q[2].length) || goog.string.compareElements_(l[2], q[2])
		} while (0 == c)
	}
	return c
};
...
goog.ui.ControlRenderer.prototype.decorate = function(a, b) {
	...
	g && h && !n && !k || goog.dom.classlist.set(b, m.join(" "));
	return b
};
...
Blockly.Bubble.prototype.renderArrow_ = function() {
	...
	if(b == d && c == e) a.push("M " + b + "," + c);
		...
		var k = Math.sin(h),
			...
			h = Math.min(h, n.width, n.height) / 2,
...
Blockly.Options = function(a) {
	...
	var l = a.toolboxPosition,
		l = "end" === l ? !1 : !0,
		l = n ? l ? Blockly.TOOLBOX_AT_TOP : Blockly.TOOLBOX_AT_BOTTOM : l ==
...
Blockly.BlockSvg.prototype.renderCompute_ = function(a) {
	...
	for(var d = 0, e = 0, f = !1, g = !1, h = !1, k = void 0, m = this.getInputsInline() && !this.isCollapsed(), n = 0, l; l = b[n]; n++)
		if(l.isVisible()) {
			var q;
			m && k && k != Blockly.NEXT_STATEMENT && l.type != Blockly.NEXT_STATEMENT ? q = c[c.length - 1] : (k = l.type, q = [], q.type = m && l.type !=
				Blockly.NEXT_STATEMENT ? Blockly.BlockSvg.INLINE : l.type, q.height = 0, c.push(q));
			q.push(l);
			...
			if(l.connection && l.connection.isConnected()) {
				var p = l.connection.targetBlock().getHeightWidth();
				l.renderHeight = Math.max(l.renderHeight, p.height);
				l.renderWidth = Math.max(l.renderWidth, p.width)
			}
			m || n != b.length - 1 ? !m && l.type == Blockly.INPUT_VALUE && b[n + 1] && b[n + 1].type == Blockly.NEXT_STATEMENT &&
				l.renderHeight-- : l.renderHeight--;
			q.height = Math.max(q.height, l.renderHeight);
			l.fieldWidth = 0;
			1 == c.length && (l.fieldWidth += this.RTL ? -a : a);
			for(var p = !1, r = 0, t; t = l.fieldRow[r]; r++) {
				0 != r && (l.fieldWidth += Blockly.BlockSvg.SEP_SPACE_X);
				var u = t.getSize();
				t.renderWidth = u.width;
				t.renderSep = p && t.EDITABLE ? Blockly.BlockSvg.SEP_SPACE_X : 0;
				l.fieldWidth += t.renderWidth + t.renderSep;
				q.height = Math.max(q.height, u.height);
				p = t.EDITABLE
			}
			q.type != Blockly.BlockSvg.INLINE && (q.type == Blockly.NEXT_STATEMENT ? (g = !0, e = Math.max(e, l.fieldWidth)) :
				(q.type == Blockly.INPUT_VALUE ? f = !0 : q.type == Blockly.DUMMY_INPUT && (h = !0), d = Math.max(d, l.fieldWidth)))
		}
	for(a = 0; q = c[a]; a++)
...
Blockly.BlockSvg.prototype.renderDrawRight_ = function(a, b, c, d, e, f, g) {
	for(var h, k = 0, m, n, l = 0, q; q = f[l]; l++) {
		...
		if(this.isCollapsed()) {
			...
			this.width = Math.max(this.width, f.statementEdge + p.connection.targetBlock().getHeightWidth().width)), l == f.length - 1 || f[l + 1].type == Blockly.NEXT_STATEMENT) && (a.push("v", Blockly.BlockSvg.SEP_SPACE_Y), this.RTL && b.push("v", Blockly.BlockSvg.SEP_SPACE_Y - 1), k += Blockly.BlockSvg.SEP_SPACE_Y);
		k += q.height
	}
...
Blockly.BlockSvg.prototype.renderDrawLeft_ = function(a, b, c) {
...
Blockly.Toolbox.prototype.populate_ = function(a) {
	function b(a, g, h) {
		for(var k = null, m = 0, n; n = a.childNodes[m]; m++)
			if(n.tagName) switch(n.tagName.toUpperCase()) {
				case "CATEGORY":
					k = c.createNode(n.getAttribute("name"));
					k.blocks = [];
					g.add(k);
					var l = n.getAttribute("custom");
					l ? k.blocks = l : b(n, k, h);
					l = n.getAttribute("colour");
					goog.isString(l) ? (l.match(/^#[0-9a-fA-F]{6}$/) ? k.hexColour = l : k.hexColour = Blockly.hueToRgb(l), e = !0) : k.hexColour = "";
					"true" == n.getAttribute("expanded") ? (k.blocks.length && c.setSelectedItem(k), k.setExpanded(!0)) :
						k.setExpanded(!1);
					k = n;
					break;
				case "SEP":
					k && ("CATEGORY" == k.tagName.toUpperCase() ? g.add(new Blockly.Toolbox.TreeSeparator(d.treeSeparatorConfig_)) : (n = parseFloat(n.getAttribute("gap")), isNaN(n) || (l = parseFloat(k.getAttribute("gap")), n = isNaN(l) ? n : l + n, k.setAttribute("gap", n))));
					break;
				case "BLOCK":
				case "SHADOW":
					g.blocks.push(n), k = n
		...
		if(Blockly.dragMode_ == Blockly.DRAG_NONE) {
			...
			if(a.contentTop <
				...
				for(var g = c.getTopBlocks(!1), h = 0, k; k = g[h]; h++) {
					...
					0 > l && k.moveBy(0, l);
					l = 25 + e - m.x - (b.RTL ? 0 : n.width);
					0 < l && k.moveBy(l, 0);
					m = e + a.viewWidth - 25 - m.x + (b.RTL ? n.width : 0);
					0 > m && k.moveBy(m, 0)
				}
		}
	});
